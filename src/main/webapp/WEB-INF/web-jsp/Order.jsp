		...
		<div class="public_m1">
			...
			<div class="public_m4">
				...
				<c:forEach items="${ordertable}" var="ordertable">
					...
					<!-- 判断是否是删除过的订单 -->
					<c:choose>
						<c:when test="${ordertable.orderStat=='5'}">
									...
									<div class="movie_box">
										<ul class="wjdc_list">
											...
											<li><b style="color: red">订单状态:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
													<c:choose>
														<c:when test="${ordertable.orderStat=='1'}">已下单</c:when>
														<c:when test="${ordertable.orderStat=='2'}">已发货</c:when>
														<c:when test="${ordertable.orderStat=='3'}">待取消</c:when>
														<c:when test="${ordertable.orderStat=='4'}">订单完成 -(可删除)</c:when>
														<%--      <c:otherwise>
         									订单完成
         						</c:otherwise>
         			 --%>
													</c:choose> <%-- ${ordertable.orderStat} --%>
											</b></li>
										</ul>
